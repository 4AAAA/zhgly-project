						...
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="编号/姓名/手机/门店" class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" placeholder="输入编号或手机关键词"/>
								...
								<td style="padding-left:5px">
									<select class="chosen-select form-control" name="LEVEL" id="LEVEL" data-placeholder="客户级别" style="vertical-align:top;width:120px;" >
										<option value=""></option>
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<li>
																<a style="cursor:pointer;" onclick="view('${var.COMPANY_ID}');" class="tooltip-success" data-rel="tooltip" title="查看">
																	<span class="green">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.COMPANY_ID}');" class="tooltip-success" data-rel="tooltip" title="编辑">
																	<span class="green">
...
<%-- 未开放批量删除功能				    <c:if test="${QX.del == 1 }">
	...
	<script type="text/javascript">
		...
		//跟踪记录
		function chaImg(COMPANY_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="订单跟踪";
			 diag.URL = '<%=basePath%>customer/listCompany.do?COMPANY_ID='+COMPANY_ID;
			 diag.Width = 820;
			 diag.Height = 600;
			 diag.Modal = false;			//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					$.get(url,function(data){
						if("success" == data.result){
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>您的客户还存在订单记录，请删除订单后操作删除客户操作!</span>",
								buttons: 			
								{
									"button" :
									{
