			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="piaoju/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text"  class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" placeholder="出货单编号"/>
...
<%-- 								<td style="padding-left:2px;"><input class="span10 date-picker" name="lastStart" id="lastStart"  value="${pd.lastStart }" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;height:34px;" placeholder="开始日期" title="开始日期"/></td>
								...
								<td style="vertical-align:top;padding-left:20px;padding-top:2px">
									<c:if test="${QX.add == 1 }">
										<a class="btn btn-white btn-success btn-round" onclick="add();"><i class="ace-icon glyphicon glyphicon-plus"></i>新增出货单</a>
									</c:if>
								</td>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.PIAOJU_CODE}</td>							
											<td class='center'>${var.DANWEI}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs action-buttons">
													<a class="blue" title="查看出货单" onclick="view('${var.PIAOJU_ID}');">
														<i class="ace-icon fa fa-search-plus bigger-130" title="查看出货单"></i>
													</a> 
													<c:if test="${QX.edit == 1 }">
													<a class="green" title="编辑" onclick="edit('${var.PIAOJU_ID}');">
														<i class="ace-icon fa fa-pencil bigger-130" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="red" onclick="del('${var.PIAOJU_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
														   <li>
																<a style="cursor:pointer;" onclick="view('${var.PIAOJU_ID}');" class="tooltip-info" data-rel="tooltip" title="查看出货单">
																	<span class="blue">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.PIAOJU_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.PIAOJU_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
...
<%-- 									<c:if test="${QX.add == 1 }">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增出货单";
			 diag.URL = '<%=basePath%>piaoju/goAdd.do';
			 diag.Width = 900;
			 diag.Height = 800;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>piaoju/delete.do?PIAOJU_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						if("success" == data.result){
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>删除操作异常!</span>",
								buttons: 			
								{
									"button" :
									{
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>piaoju/goEdit.do?PIAOJU_ID='+Id;
			 diag.Width = 900;
			 diag.Height = 800;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function view(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="查看出货单";
			 diag.URL = '<%=basePath%>piaoju/goView.do?PIAOJU_ID='+Id;
			 diag.Width = 900;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
