...
public class LoginController extends BaseController {
	...
	public ModelAndView defaultPage() throws Exception{
		...
		String str1 = "";
		String str2 = "";
		str1 += "<graph decimals='2' caption='前12个月销售额柱状图' xAxisName='月份' yAxisName='元' decimalPrecision='0' formatNumberScale='0'>";
		str2 += "<graph decimals='2' caption='前12个月销售额折线图' xAxisName='月份' yAxisName='元' decimalPrecision='0' formatNumberScale='0'>";
		String strY = new java.text.SimpleDateFormat("yyyy").format(new Date());
		String strM = new java.text.SimpleDateFormat("MM").format(new Date());
		String str = "";
		String pstr = "";
		String[] color = {"AFD8F8","F6BD0F","8BBA00","FF8E46","008E8E","D64646","8E468E","588526","B3AA00","008ED6","9D080D","A186BE"};
		int nowY = Integer.parseInt(strY);
		int nowM = Integer.parseInt(strM);
		for(int i=0; i<12; i++){
			
			if(nowM < 10 && nowM >= 1){
				strM = "0" + String.valueOf(nowM);
			}else if(nowM < 1){
				nowY = nowY - 1;
				strY = String.valueOf(nowY);
				nowM = 12;
				strM = String.valueOf(nowM);
			}else{
				strM = String.valueOf(nowM);
			} 
			str = strY + "-" + strM;
			ympd.put("onemonth", str);
			PageData onemonthpd = outkuService.priceSum(ympd);
			String onemonthJine = "0";
			if(null != onemonthpd){
				onemonthJine = onemonthpd.get("ZPRICE").toString();
			}
			pstr = "<set name='" + str + "' value='"+onemonthJine+"' color='"+color[i]+"'/>" + pstr;
			nowM--;
		}
		str1 = str1 + pstr;
		str1 += "</graph>";
